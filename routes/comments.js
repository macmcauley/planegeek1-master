    ...
    Plane.findById(req.params.id, function(err, plane){
        if(err){
            console.log(err);
             ...
             res.render("comments/new", {plane: plane});
        }
    });
});
...
router.get("/:comment_id/edit", middleware.checkCommentOwnership, function(req, res){
    Plane.findById(req.params.id, function(err, foundPlane) {
        if(err || !foundPlane){
            req.flash("error", "No plane found");
        ...
        Comment.findById(req.params.comment_id, function(err, foundComment){
            if(err){
                ...
                res.render("comments/edit", {plane_id: req.params.id, comment: foundComment});
            }
        });
    });
});
